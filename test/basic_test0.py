...
def basic_test(board_id, file):
    with MbedBoard.chooseBoard(board_id=board_id) as board:
        ...
        print "\r\n\r\n------ TEST PROGRAM/ERASE LESS THAN A PAGE------"
        # Fill pages with 0x55, 1/4 page at a time
        page_size = flash.getPageInfo(addr_flash).size
        fill = [0x55] * (page_size/4)
        flash.init()
        for i in range(0, 3):
            address = addr_flash + page_size * i
            # Test only supports a location with 3 aligned
            ...
            assert page_size == current_page_size
            assert address % current_page_size == 0
            print "\n erasing page at 0x%X" % address
            flash.erasePage(address)
            for j in range (0,4):
                print "\n programming 0x%X bytes at 0x%X" % ((page_size/4), address + (j* page_size/4))
                flash.programPage(address + (j * page_size/4) , fill)
